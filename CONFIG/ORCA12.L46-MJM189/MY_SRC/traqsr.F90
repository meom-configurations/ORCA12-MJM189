MODULE traqsr
   ...
   !!            3.4  !  2013-02  (R. Bourdalle-Badie) Add 2d file for 2 band scheme
   !!----------------------------------------------------------------------
   ...
   LOGICAL , PUBLIC ::   ln_chl_read= .TRUE.    !: In 2 band case read chlorophyll field or attenuation depth
...
!$AGRIF_DO_NOT_TREAT
   TYPE(FLD), PUBLIC, ALLOCATABLE, DIMENSION(:) ::   sf_chl   ! structure of input Chl (file informations, fields read)
...
CONTAINS
   ...
   SUBROUTINE tra_qsr( kt )
      ...
      !!              Murtugudde R. at al., 2002, J. of Climate, p470-486
      ...
      REAL(wp), POINTER, DIMENSION(:,:  ) :: zxsi1r_2d                      ! transform xsi1r in 2D array
      ...
      !
      CALL wrk_alloc( jpi, jpj,      zxsi1r_2d               ) 
...
!CDIR NOVERRCHK   
         ...
         IF( ln_qsr_2bd ) THEN                            !  2 band light penetration !
            !                                             ! ------------------------- !
            !
            IF( lk_vvl .OR. nn_chldta == 1 ) THEN         !* variable volume or read an input file
               IF( nn_chldta == 1 ) THEN                  ! Read Chl data and provides it at the current time step
                  CALL fld_read( kt, 1, sf_chl ) 
                  IF( ln_chl_read ) THEN   ! Read chloro. values and transform in att. depth. (Murtugudde 2002)
                     zxsi1r_2d(:,:) = 1./37. + 0.0518 * sf_chl(1)%fnow(:,:,1)**0.428                  
                  ELSE                     ! Attenuation Depth is read
                     zxsi1r_2d(:,:) = 1. / sf_chl(1)%fnow(:,:,1)
                  ENDIF
               ELSE                        ! Constant namelist value
                  zxsi1r_2d(:,:) = xsi1r
               ENDIF
               !zz0   =        rn_abs   * ro0cpr
               !zz1   = ( 1. - rn_abs ) * ro0cpr
               ...
               DO jk = 1, nksr                    ! solar heat absorbed at T-point in the top 400m 
                  DO jj = 1, jpj
                     DO ji = 1, jpi
                        zc0 = zz0 * EXP( -fsdepw(ji,jj,jk  )*xsi0r ) + zz1 * EXP( -fsdepw(ji,jj,jk  )*zxsi1r_2d(ji,jj) )
                        zc1 = zz0 * EXP( -fsdepw(ji,jj,jk+1)*xsi0r ) + zz1 * EXP( -fsdepw(ji,jj,jk+1)*zxsi1r_2d(ji,jj) )
      ...
      !
      CALL wrk_dealloc( jpi, jpj,      zxsi1r_2d               ) 
   ...
   SUBROUTINE tra_qsr_init
      ...
      !!
      NAMELIST/namtra_qsr/  sn_chl, cn_dir, ln_traqsr, ln_qsr_rgb, ln_qsr_2bd, ln_chl_read, ln_qsr_bio,   &
      ...
      IF(lwp) THEN                ! control print
         ...
         WRITE(numout,*) '      RGB & 2 bands: Chl data (=1) or cst value (=0)                     nn_chldta  = ', nn_chldta
         ...
         WRITE(numout,*) '      2 bands: longest depth depending on Chloro (T) or att.depth (F)   ln_chl_read = ', ln_chl_read
         WRITE(numout,*) '      2 bands: longest depth of extinction (if constant)                     rn_si1 = ', rn_si1
      ENDIF
      ...
      IF( ln_traqsr ) THEN     ! control consistency
         ...
         IF( ln_qsr_2bd .AND. nn_chldta == 0 )                           nqsr =  3
         IF( ln_qsr_2bd .AND. nn_chldta == 1 .AND. ln_chl_read )         nqsr =  4
         IF( ln_qsr_2bd .AND. nn_chldta == 1 .AND. .NOT. ln_chl_read )   nqsr =  5
         ...
         IF(lwp) THEN                   ! Print the choice
            ...
            IF( nqsr ==  3 )   WRITE(numout,*) '         2 bands light penetration - Constant Att. Depth'
            IF( nqsr ==  4 )   WRITE(numout,*) '         2 bands light penetration - Chl data'
            IF( nqsr ==  5 )   WRITE(numout,*) '         2 bands light penetration - Att. Depth data'
      ...
      IF( ln_traqsr  ) THEN      !  Initialisation of Light Penetration  !  
         ...
         xsi0r = 1.e0 / rn_si0
         xsi1r = 1.e0 / rn_si1
         IF( nn_chldta == 1 ) THEN           !* Read Chl or Att. depth data : set sf_chl structure
            IF(lwp) WRITE(numout,*)
            IF(lwp) WRITE(numout,*) '        Chlorophyll or Att. Depth read in a file'
            ...
            CALL fld_fill( sf_chl, (/ sn_chl /), cn_dir, 'tra_qsr_init',   &
               &                                         'Solar penetration function of read chloro or Att. Depth', 'namtra_qsr' )
         ENDIF
         !                                ! ---------------------------------- !
         IF( ln_qsr_rgb ) THEN            !  Red-Green-Blue light penetration  !
            ...
            !
            IF( nn_chldta == 1 .AND. .NOT. ln_chl_read) THEN     !* Att. depth data not allowed with rgb scheme 
               IF(lwp) WRITE(numout,*)
               CALL ctl_stop( 'tra_qsr_init: Chlorophyll values should be read with RGB light penetration' )   ;   RETURN
               !
            ELSEIF(nn_chldta == 0) THEN         !* constant Chl : compute once for all the distribution of light (etot3)
               IF(lwp) WRITE(numout,*)
...
!CDIR NOVERRCHK   
         ...
         IF( ln_qsr_2bd ) THEN            !    2 bands    light penetration    !
            ...
            !
            !                                ! level of light extinction
            !rbb nksr = trc_oce_ext_lev( rn_si1, 1.e2 )
            nksr = trc_oce_ext_lev( 50., 1.e2 ) !rbb Ugly Patch: light extinction forced at 50m !
            IF(lwp) THEN
               WRITE(numout,*)
            IF(lwp) WRITE(numout,*) '        level of light extinction = ', nksr, ' ref depth = ', gdepw_0(nksr+1), ' m'
            ENDIF
            ...
            IF( lk_vvl ) THEN                     ! variable volume 
               IF(lwp) WRITE(numout,*) '        key_vvl: light distribution will be computed at each time step'
            ELSE
               IF(nn_chldta == 0 )  THEN          ! constant volume and no input field (chl constant): computes one for all
                  !zz0 =        rn_abs   * ro0cpr
                  !zz1 = ( 1. - rn_abs ) * ro0cpr
                  zz0 =        rn_abs   * r1_rau0_rcp
                  zz1 = ( 1. - rn_abs ) * r1_rau0_rcp
